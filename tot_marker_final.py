...
#azzera numero sentences con concorrenza lemmi modali pari o superiore a 1 (selezionate per l'output)
...
#		print (token["form"])
			...
			if (j == (len(sentence)-1)): #se siamo all'ultimo token. NB ricontrollo se j non è l'ultimo token ma l'avevo già fatto in 114
				...
				#########################################################################
				#########################################################################
				lista_lemmi_in_sentence = lista_lemmi_in_sentence + token["form"] + ' ' + token_2["form"] + ',' #sequenza_form è stata già aggiornata
		...
		if (token["lemma"]) == 'aequus': #token["lemma"] viene elaborato qui
			...
			if (j == (len(sentence)-1)):
				...
				lista_lemmi_in_sentence = lista_lemmi_in_sentence + token["form"] + ' ' + token_2["form"] + ','
			else:
	...
	if (cocorrenza > 0): #per contare tutte le occorrenze
...
f = open(nome_file_senza_estensione + '_output' + '.tot_markers', 'w') #apre un file in scrittura 'w' (lettura 'r')
...
dict = {'Numero sentences totale:                    ' : len(sentences), 'Numero sentences con 0 lemmi modali:        ' : n_sentences_0l, 'Numero sentences con 1 lemma modale:        ' : n_sentences_1l,\
  'Numero sentences con 2 lemmi modali:        ' : n_sentences_2l, 'Numero sentences con 3 o piu\' lemmi modali: ' : n_sentences_3l, 'Numero frasi trovate (almeno 1 occorrenza): ' :n_output_sentences}
...
print ('# Numero frasi trovate (almeno 1 occorrenza: ', n_output_sentences)
print ('#')
